...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	speed := float64(CaloriesMeanSpeedMultiplier)*r.meanSpeed() + CaloriesMeanSpeedShift
	return (speed * r.Weight / float64(MInKm) * float64(r.Duration.Hours()) * float64(MinInHours))
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	speedPow := math.Pow(w.meanSpeed()*KmHInMsec, 2)
	return ((CaloriesWeightMultiplier*w.Weight + (speedPow/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
